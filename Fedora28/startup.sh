...
if [ -n "$SATHOST" ]; then
    ...
    curl --insecure --output katello-ca-consumer-latest.noarch.rpm https://$SATHOST/pub/katello-ca-consumer-latest.noarch.rpm
...
dnf -y install katello-host-tools

# if the KILL arg was not passed, then keep the container running
if [ -z "$KILL" ]; then
    echo "Setting up and starting sshd"
    ssh-keygen -A
    /usr/sbin/sshd
